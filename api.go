...
func main() {
	...
	log.Infof("ManageF: Version: %v, Commit: %v\n", version, commitHash)
	log.Debugf("ManageF: Command line: [%v]", strings.Join(os.Args, " "))
	...
	log.Tracef("ManageF Configuration:\n%s", config.Get())

	if err := validateConfig(); err != nil {
		glog.Fatal(err)
	}

	log.Infof("ManageF: Console version: %v", determineConsoleVersion())
